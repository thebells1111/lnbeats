...
async function downloadImage(url, index) {
  ...
  const filePath = path.join(IMAGE_FOLDER, hashedFilename);
  ...
  try {
    ...
    if (contentType.startsWith("image/gif")) {
      // const isAnimated = await checkGifAnimation(data);
      // if (!isAnimated) return await processImage(data, filePath);
      // console.log(`Skipping animated GIF: ${url}`);
      return;
    }

    await processImage(data, filePath);
    console.log(`Saved: ${index} of ${length}`);
  } catch (err) {
